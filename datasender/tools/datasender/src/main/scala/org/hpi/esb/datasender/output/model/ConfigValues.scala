...
object ConfigValues {
  ...
  val header = List(BATCH_SIZE, BUFFER_MEMORY_SIZE, LINGER_TIME, READ_IN_RAM, SENDING_INTERVAL, SENDING_INTERVAL_TIMEUNIT, SCALE_FACTOR)
  ...
  def get(config: Config, benchmarkConfig: BenchmarkConfig): ConfigValues = {
    ConfigValues(
      ...
      scaleFactor = benchmarkConfig.scaleFactor.toString)
  }
}
...
case class ConfigValues(batchSize: String, bufferMemorySize: String, lingerTime: String,
  ...
  def this(m: Map[String, String]) = this(m(BATCH_SIZE), m(BUFFER_MEMORY_SIZE), m(LINGER_TIME),
      m(READ_IN_RAM), m(SENDING_INTERVAL), m(SENDING_INTERVAL_TIMEUNIT), m(SCALE_FACTOR))

  def toList(): List[String] = {
    List(batchSize.toString, bufferMemorySize.toString, lingerTime.toString, readInRam.toString,
      sendingInterval.toString, sendingIntervalTimeUnit, scaleFactor)
  }
}
