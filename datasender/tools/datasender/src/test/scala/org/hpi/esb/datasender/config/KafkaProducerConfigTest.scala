...
class KafkaProducerConfigTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  "KafkaProducerConfig.isValid" should "return false if attribute bootstrapServers is empty" in {
    val invalidServer = None
    val config = KafkaProducerConfig(invalidServer, keySerializer, valueSerializer, acks, batchSize)
  ...
  it should "return false if attribute keySerializer is empty" in {
    ...
    val config = KafkaProducerConfig(server, invalidKeySerializer, valueSerializer, acks, batchSize)
    config.logger = mock[Logger]
  ...
  it should "return false if attribute valueSerializer is empty" in {
    ...
    val config = KafkaProducerConfig(server, keySerializer, invalidValueSerializer, acks, batchSize)
    config.logger = mock[Logger]
  ...
  it should "return false if attribute acks is empty" in {
    ...
    val config = KafkaProducerConfig(server, keySerializer, valueSerializer, invalidAcks, batchSize)
    config.logger = mock[Logger]
  ...
  it should "return true if acks == 1" in {
    ...
    val config = KafkaProducerConfig(server, keySerializer, valueSerializer, validAcks, batchSize)
    config.logger = mock[Logger]
  ...
  it should "return true if acks == 0" in {
    val config = KafkaProducerConfig(server, keySerializer, valueSerializer, acks, batchSize)
    config.logger = mock[Logger]
  ...
  it should "return false if batch size < 0" in {
    ...
    val config = KafkaProducerConfig(server, keySerializer, valueSerializer, acks, invalidBatchSize)
    config.logger = mock[Logger]
